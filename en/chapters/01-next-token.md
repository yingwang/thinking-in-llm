...
If you can remember only one sentence from this book, let it be this: **large language models do one thing and only one thing: predict the next token**.

All the dazzling abilities you hear about in LLMs, such as writing poetry, programming, reasoning, and translation, are not functions that were specially programmed into them. They are byproducts that **emerge** from this extremely simple objective. Understand this, and you understand the foundation of LLMs.
...
Given all previous tokens, the model outputs a probability distribution representing what the next token is most likely to be. That is all. There is no "understanding" module, no "reasoning" engine, no "knowledge-base query", only this one probability distribution.
...
The model selects a token, such as "Paris", appends it to the input, and then predicts the next one. This loops until an end-of-sequence marker (EOS) is generated or a length limit is reached. This is **autoregressive generation**.

### Training: Maximizing Likelihood over Trillions of Tokens

The training process is just as concise: show the model a piece of real text, ask it to predict the next token at each position, and then use cross-entropy loss to measure how accurate the predictions are:

$$\mathcal{L} = -\sum_{t=1}^{T} \log P(x_t \mid x_1, x_2, \ldots, x_{t-1})$$

This objective function is called **maximum log-likelihood**. The model repeatedly optimizes this objective over text containing trillions of tokens: Wikipedia, books, code, papers, web pages, and almost the sum total of human civilization in written form.
...
The answer is: **to truly predict the next token well, you need to understand the structure behind the text**.
...
In other words, in order to predict a token, the model is forced to build some kind of internal representation of world knowledge, grammatical structure, and logical relationships. Ilya Sutskever put it succinctly in a talk:
...
When you input "artificial intelligence", the model does not see two words. It may see two tokens, `[artificial, intelligence]`, or three tokens, `[art, ificial, intelligence]`, depending on the tokenizer's vocabulary.
...
Why? Because the tokenizer splits "strawberry" into tokens similar to `["str", "aw", "berry"]`. The model has never "seen" each independent letter. It processes token-level sequences. Asking it to count letters is like asking you to count how many chairs are in a room while blindfolded.
...
Tokenizers are usually trained on primarily English corpora. This leads to an important asymmetry:
...
- The same semantics consume more tokens → **more expensive** (APIs charge by token)
- The context window can fit less Chinese content
...
Mathematically, temperature does something simple: it divides by a scalar before softmax:
...
Do not treat temperature as a "hyperparameter that needs tuning". Switch to another mental model: **you are choosing the model's thinking style**:
...
Top-p sampling works differently: instead of selecting a fixed number of candidate tokens, it selects the tokens whose probabilities, accumulated from high to low, reach p.
...
- When the model is very certain, such as when one token has probability 0.95, the candidate set has only 1-2 tokens
- When the model is uncertain and probabilities are dispersed, the candidate set automatically expands

### Top-k: Hard Truncation

Top-k is more blunt: it only keeps the k tokens with the highest probabilities.
...
A freshly trained model, a base model, is a continuation engine. Whatever you input, it continues from there:
...
To make the model "converse", we need a **chat template**. The core idea is to use special tokens to format the input as a conversation, so that the model's continuation just happens to be an answer.
...
When the model sees this format, based on large amounts of similarly formatted dialogue in the training data, it will continue with:
...
A system prompt is not an "instruction". It is **conditional information that changes the probability landscape**. The model does not have a module for "following instructions". It is only continuing the most likely tokens under the condition of the system prompt.

This explains why:
- A very long system prompt may be less effective than a short and precise one, because the signal is diluted
- Content later in the system prompt often has more influence, due to recency bias
- Format and wording matter more than semantics, because the model matches patterns in the training data
...
At every step, the model is doing only one thing: given the preceding text, predict the next token. There is no step for "understanding the input", no step for "thinking of the answer", and no step for "organizing language". All these "intelligent behaviors" that we perceive are emergent effects of next-token prediction at a sufficiently large model scale and over sufficiently much data.

---

## 1.5 Thought Experiment: Can a Next-Token Predictor Understand Language?

In this section, we temporarily leave engineering and enter philosophy. This is not for showing off. It is because **your answer to this question profoundly affects how you design systems and how much you trust model outputs**.
...
Marcus Hutter, the creator of AIXI, once established a **compression prize** (Hutter Prize): whoever can better compress Wikipedia better understands human knowledge.
...
For example, the model may have learned that "Marie Curie" is often followed by "radium" and "Nobel", rather than truly understanding radioactive physics. It is doing "parroting" (stochastic parrot, [Bender et al. 2021](https://dl.acm.org/doi/10.1145/3442188.3445922)), merely statistical pattern matching.
...
2. **But do not underestimate it either**. This mathematical function has indeed built some kind of internal representation of a world model. Later chapters will show evidence.
...
- **The model will gracefully degrade outside the training distribution**, because it matches patterns, and when it has not seen a pattern, it guesses incorrectly
